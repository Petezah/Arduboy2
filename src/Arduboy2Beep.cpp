...
#if !defined(AB_DEVKIT) && !defined(ARDUBOY_TINY)
...
// Speaker pin 1, Timer 0B, Port B bit 1
...
{
  duration = dur;
  TCCR0A = bit(COM0B0); // set toggle on compare mode (which connects the pin)
  OCR0B = count; // load the count (16 bits), which determines the frequency
}

void BeepPin1::timer()
{
  if (duration && (--duration == 0)) {
    TCCR0A = 0; // set normal mode (which disconnects the pin)
  }
}
...
// Speaker pin 2 is not present on Tiny
